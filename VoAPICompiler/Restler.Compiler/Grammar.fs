...
module Tree =
    type Tree<'LeafData,'INodeData> =
        ...
        override this.ToString() =
            let rec toString (tree:Tree<'LeafData,'INodeData>) =
                match tree with
                | LeafNode leafInfo ->
                    sprintf "LeafNode %O" leafInfo
                | InternalNode (nodeInfo,subtrees) ->
                    sprintf "InternalNode (%O, seq [ %s ] )" nodeInfo
                        (subtrees |> Seq.map toString |> String.concat ";\n ")
            toString this
...
type RequestParametersPayload =
    ...
    override this.ToString() =
        ...
        | ParameterList parameters ->
            parameters
            |> Seq.map (fun p -> sprintf "%O" p )
            |> String.concat ";\n "
            |> sprintf "ParameterList \n ( seq [ %s ] )"
        | Example payload ->
            payload.ToString()
...
type RequestParameters =
    ...
    override this.ToString() = 
        let headerStr = 
            this.header
            |> Seq.map (fun (src, payload) -> sprintf "(%A, %O)" src payload)
            |> String.concat ";\n "
            |> sprintf "[ %s ]"
        let queryStr =
            this.query
        ...
        let bodyStr =
            ...
            |> sprintf "[ %s ]"
        sprintf "{path = %O \n header = %s \n query = %s \n body = %s }"
            this.path headerStr queryStr bodyStr
